...
function KpiCard({ label, value, sub, dot, spark, onClick, active, T }) {
  const [hov, setHov] = useState(false);
  return (
    <div onClick={onClick}
      style={{
        background: active ? `${dot}12` : T.surface,
        border: `1.5px solid ${active ? dot : T.line}`,
        borderRadius: 12,
        ...
        boxShadow: active ? `0 4px 16px ${dot}22` : hov && onClick ? T.shadowMd : T.shadowSm,
      }}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}>
      {/* Accent bar */}
      <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: 3, background: dot, borderRadius: "12px 12px 0 0", opacity: active ? 1 : 0.4 }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontSize: 10, fontWeight: 600, color: T.inkMuted, fontFamily: F_MONO, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 10 }}>{label}</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: active ? dot : T.ink, fontFamily: F, letterSpacing: "-1px", lineHeight: 1 }}>{value}</div>
          <div style={{ fontSize: 11, color: active ? dot : T.inkMuted, marginTop: 7, display: "flex", alignItems: "center", gap: 4 }}>
...
export function Dashboard({ funnels, user, onView, onAdd, statFilter, onStatClick, recentlyViewed, T }) {
  ...
  const kpis = [
    { label:"Total Leads",    value:funnels.length, sub:"All time",           dot:T.inkMuted,    filterKey:null,      spark:null },
    { label:"Won Deals",      value:won.length,     sub:`${wr}% win rate`,    dot:T.won.dot,     filterKey:"Won",     spark:wonSpark },
    { label:"Pending",        value:pending.length, sub:"Need follow-up",     dot:T.pending.dot, filterKey:"Pending", spark:null },
    { label:"Overdue",        value:overdue.length, sub:"Follow-ups missed",  dot:T.lost.dot,    filterKey:null,      spark:null },
    { label:"Won Revenue",    value:big(wonRev),    sub:"Closed revenue",     dot:T.won.dot,     filterKey:"Won",     spark:null },
    { label:"Pipeline Value", value:big(pipeline),  sub:"Potential revenue",  dot:T.pending.dot, filterKey:"Pending", spark:null },
  ];
  ...
  return (
    <div style={{ padding: "20px 20px 32px", display: "flex", flexDirection: "column", gap: 16, maxWidth: 1600, width: "100%" }}>
      ...
      {/* ── KPI STRIP ── */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 12 }} className="ek-kpi-grid">
        {kpis.map((k,i) => (
