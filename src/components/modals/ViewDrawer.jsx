...
export function ViewDrawer({
  ...
  const handlePrint = () => {
    ...
    <p style="margin-top:40px;font-size:10px;color:#aaa">Printed from Suntronix CRM · ${new Date().toLocaleString("en-IN")}</p>
    </body></html>`);
    w.document.close();
  ...
  // Helpers
  const Row = ({ l, v, mono }) => (
    <div style={{ display:"grid", gridTemplateColumns:"140px 1fr", gap:8, padding:"9px 0", borderBottom:`1px solid ${T.line}` }}>
  ...
  return (
    <>
      {/* ── BACKDROP ── */}
      <div
        style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:9000, display:"flex", justifyContent:"flex-end", backdropFilter:"blur(3px)" }}
        onClick={onClose}>

        {/* ── PANEL ── */}
        <div
          style={{ background:T.surface, width:"100%", maxWidth:"min(600px,100vw)", height:"100%", display:"flex", flexDirection:"column", boxShadow:"-12px 0 48px rgba(0,0,0,.25)", animation:"slideRight .22s ease" }}
          onClick={e => e.stopPropagation()}>

          {/* ─── STICKY HEADER ─── */}
          <div style={{ padding:"18px 22px 14px", borderBottom:`1px solid ${T.line}`, background:T.surface, flexShrink:0, position:"sticky", top:0, zIndex:10 }}>
